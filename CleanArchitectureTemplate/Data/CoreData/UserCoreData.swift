...
//                guard let login = favoriteUser.login , let imageURL = favoriteUser.imageURL else { return nil }
    ...
    public func deleteFavoritreUser(userId: Int) -> Result<Bool, CoreDataError> {
        let fetchRequsest:NSFetchRequest<FavoriteUser> = FavoriteUser.fetchRequest()
        fetchRequsest.predicate = NSPredicate(format: "id == %d", userId)
        
        do {
            let result = try viewContext.fetch(fetchRequsest)
